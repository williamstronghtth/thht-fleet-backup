#!/usr/bin/env python3
"""Sabal Creek Absentee - Touch 1 Email follow-up to Chris's cold calls"""
...
SUBJECT = "Following up on Chris's call about your Sabal Creek property"
TEMPLATE = """Hi {first_name},

My team lead Chris gave you a call today about your property in Sabal Creek. He wanted to connect since you own there but live out of the area.
...
SKIP_EMAILS = {
    "iwafik@hotmail.com",          # Ibrahim Wafik - already in Waters Edge
    "jreader@pyramidhotels.com"    # Jason Reader - already in Sanctuary
}
...
def main():
    ...
    for l in leads:
        ...
        eligible.append(l)
    log('='*50)
    log('Sabal Creek Absentee - Touch 1')
    log('='*50)
    log(f'Total leads: {len(leads)}')
    log(f'Eligible (deduplicated, skips removed): {len(eligible)}')
    log(f'Skipped: Ibrahim Wafik (Waters Edge), Jason Reader (Sanctuary)')
    sent = errors = 0
