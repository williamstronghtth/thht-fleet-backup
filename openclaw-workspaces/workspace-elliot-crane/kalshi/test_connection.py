...
def test_connection():
    ...
    print("\n1. Testing exchange status...")
    try:
        resp = make_request("GET", "/exchange/status")
        print(f"   Status Code: {resp.status_code}")
        if resp.status_code == 200:
            data = resp.json()
            print(f"   Exchange Status: {data}")
            print("   ✅ Exchange endpoint working!")
        else:
    ...
    print("\n2. Testing account balance (auth required)...")
    try:
        resp = make_request("GET", "/portfolio/balance")
        print(f"   Status Code: {resp.status_code}")
        if resp.status_code == 200:
            data = resp.json()
    ...
    # Test 3: Get markets list
    print("\n3. Testing markets endpoint...")
    try:
        resp = make_request("GET", "/markets", params={"limit": 5})
        print(f"   Status Code: {resp.status_code}")
        if resp.status_code == 200:
            data = resp.json()
            markets = data.get("markets", [])
            print(f"   Found {len(markets)} markets (limited to 5)")
            for m in markets[:3]:
