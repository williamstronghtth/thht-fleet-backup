#!/usr/bin/env python3
"""
Kalshi API Connection Test v2
Testing different signature methods
"""

import base64
import datetime
import hashlib
import requests
from cryptography.hazmat.primitives import hashes, serialization
from cryptography.hazmat.primitives.asymmetric import padding, utils
from cryptography.hazmat.backends import default_backend

# API Configuration
API_KEY_ID = "adcd595f-ac7a-4d70-b6df-a859a2f3ac63"

PRIVATE_KEY_PEM = """-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----"""


def load_private_key():
    return serialization.load_pem_private_key(
        PRIVATE_KEY_PEM.encode(),
        password=None,
        backend=default_backend()
    )


# Try different API base URLs
API_BASES = [
    "https://api.elections.kalshi.com/trade-api/v2",
    "https://trading-api.kalshi.com/trade-api/v2",
    "https://api.kalshi.com/trade-api/v2",
]


def sign_pss(private_key, timestamp: str, method: str, path: str) -> str:
    """PSS signature (some APIs use this)"""
    message = f"{timestamp}{method}{path}".encode()
    signature = private_key.sign(
        message,
        padding.PSS(
            mgf=padding.MGF1(hashes.SHA256()),
            salt_length=padding.PSS.AUTO
        ),
        hashes.SHA256()
    )
    return base64.b64encode(signature).decode()


def sign_pkcs1v15(private_key, timestamp: str, method: str, path: str) -> str:
    """PKCS1v15 signature"""
    message = f"{timestamp}{method}{path}".encode()
    signature = private_key.sign(
        message,
        padding.PKCS1v15(),
        hashes.SHA256()
    )
    return base64.b64encode(signature).decode()


def sign_pkcs1v15_no_hash(private_key, timestamp: str, method: str, path: str) -> str:
    """PKCS1v15 with prehashed"""
    message = f"{timestamp}{method}{path}".encode()
    digest = hashlib.sha256(message).digest()
    signature = private_key.sign(
        digest,
        padding.PKCS1v15(),
        utils.Prehashed(hashes.SHA256())
    )
    return base64.b64encode(signature).decode()


def test_auth_method(api_base, sign_func, name):
    """Test a specific auth method"""
    private_key = load_private_key()
    timestamp = str(int(datetime.datetime.now(datetime.timezone.utc).timestamp() * 1000))
    method = "GET"
    
    # Extract just the path part for signing
    path_only = "/trade-api/v2/portfolio/balance"
    
    signature = sign_func(private_key, timestamp, method, path_only)
    
    headers = {
        "KALSHI-ACCESS-KEY": API_KEY_ID,
        "KALSHI-ACCESS-SIGNATURE": signature,
        "KALSHI-ACCESS-TIMESTAMP": timestamp,
        "Content-Type": "application/json"
    }
    
    url = f"{api_base}/portfolio/balance"
    
    try:
        resp = requests.get(url, headers=headers, timeout=10)
        print(f"   {name}: {resp.status_code}")
        if resp.status_code == 200:
            print(f"      ✅ SUCCESS! Balance: {resp.json()}")
            return True
        elif resp.status_code != 404:
            print(f"      {resp.text[:100]}")
    except Exception as e:
        print(f"   {name}: Error - {e}")
    return False


def test_alternative_endpoints():
    """Try different API endpoints and signing methods"""
    print("=" * 60)
    print("TESTING DIFFERENT API CONFIGURATIONS")
    print("=" * 60)
    
    sign_methods = [
        (sign_pkcs1v15, "PKCS1v15"),
        (sign_pss, "PSS"),
        (sign_pkcs1v15_no_hash, "PKCS1v15-prehash"),
    ]
    
    for api_base in API_BASES:
        print(f"\n🔗 Testing: {api_base}")
        for sign_func, name in sign_methods:
            if test_auth_method(api_base, sign_func, name):
                return
    
    # Also try with path variations
    print("\n" + "=" * 60)
    print("TRYING PATH VARIATIONS")
    print("=" * 60)
    
    private_key = load_private_key()
    timestamp = str(int(datetime.datetime.now(datetime.timezone.utc).timestamp() * 1000))
    method = "GET"
    
    # Different path formats to sign
    path_variations = [
        "/trade-api/v2/portfolio/balance",
        "/portfolio/balance",
        "trade-api/v2/portfolio/balance",
        "/v2/portfolio/balance",
    ]
    
    api_base = "https://api.elections.kalshi.com/trade-api/v2"
    
    for path in path_variations:
        message = f"{timestamp}{method}{path}".encode()
        signature = private_key.sign(
            message,
            padding.PKCS1v15(),
            hashes.SHA256()
        )
        sig_b64 = base64.b64encode(signature).decode()
        
        headers = {
            "KALSHI-ACCESS-KEY": API_KEY_ID,
            "KALSHI-ACCESS-SIGNATURE": sig_b64,
            "KALSHI-ACCESS-TIMESTAMP": timestamp,
        }
        
        url = f"{api_base}/portfolio/balance"
        try:
            resp = requests.get(url, headers=headers, timeout=10)
            status = "✅" if resp.status_code == 200 else "❌"
            print(f"   {status} Path '{path}': {resp.status_code}")
            if resp.status_code == 200:
                print(f"      Balance: {resp.json()}")
                return
        except Exception as e:
            print(f"   Path '{path}': Error - {e}")


if __name__ == "__main__":
    test_alternative_endpoints()
