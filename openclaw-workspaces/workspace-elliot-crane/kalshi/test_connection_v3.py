#!/usr/bin/env python3
"""
Kalshi API Connection Test v3
Testing signature format variations based on current Kalshi v2 API
"""

import base64
import datetime
import json
import requests
from cryptography.hazmat.primitives import hashes, serialization
from cryptography.hazmat.primitives.asymmetric import padding
from cryptography.hazmat.backends import default_backend

# API Configuration
API_BASE = "https://api.elections.kalshi.com/trade-api/v2"
API_KEY_ID = "adcd595f-ac7a-4d70-b6df-a859a2f3ac63"

PRIVATE_KEY_PEM = """-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----"""


def load_private_key():
    return serialization.load_pem_private_key(
        PRIVATE_KEY_PEM.encode(),
        password=None,
        backend=default_backend()
    )


def test_signature_variants():
    """Test different signature message formats"""
    print("=" * 60)
    print("TESTING SIGNATURE MESSAGE FORMATS")
    print("=" * 60)
    
    private_key = load_private_key()
    method = "GET"
    endpoint = "/portfolio/balance"
    
    # Message format variations to try
    message_formats = [
        # Format 1: timestamp + method + full path (original)
        lambda ts: f"{ts}{method}/trade-api/v2{endpoint}",
        # Format 2: timestamp + method + endpoint only  
        lambda ts: f"{ts}{method}{endpoint}",
        # Format 3: timestamp + method + path (no leading slash)
        lambda ts: f"{ts}{method}trade-api/v2{endpoint}",
        # Format 4: method + path + timestamp
        lambda ts: f"{method}/trade-api/v2{endpoint}{ts}",
        # Format 5: Including empty body
        lambda ts: f"{ts}{method}/trade-api/v2{endpoint}",
        # Format 6: Lowercase method
        lambda ts: f"{ts}get/trade-api/v2{endpoint}",
        # Format 7: With newlines
        lambda ts: f"{ts}\n{method}\n/trade-api/v2{endpoint}",
    ]
    
    for i, fmt_func in enumerate(message_formats):
        timestamp = str(int(datetime.datetime.now(datetime.timezone.utc).timestamp() * 1000))
        message = fmt_func(timestamp)
        
        signature = private_key.sign(
            message.encode(),
            padding.PKCS1v15(),
            hashes.SHA256()
        )
        sig_b64 = base64.b64encode(signature).decode()
        
        headers = {
            "KALSHI-ACCESS-KEY": API_KEY_ID,
            "KALSHI-ACCESS-SIGNATURE": sig_b64,
            "KALSHI-ACCESS-TIMESTAMP": timestamp,
            "Content-Type": "application/json"
        }
        
        url = f"{API_BASE}{endpoint}"
        
        try:
            resp = requests.get(url, headers=headers, timeout=10)
            status = "✅" if resp.status_code == 200 else "❌"
            print(f"\n{status} Format {i+1}: {resp.status_code}")
            print(f"   Message: {message[:60]}...")
            if resp.status_code == 200:
                print(f"   SUCCESS! Response: {resp.json()}")
                return True
            elif "INCORRECT" not in resp.text:
                print(f"   Response: {resp.text[:100]}")
        except Exception as e:
            print(f"\n❌ Format {i+1}: Error - {e}")
    
    # Also test with different header names (some APIs use different casing)
    print("\n" + "=" * 60)
    print("TESTING HEADER VARIATIONS")
    print("=" * 60)
    
    header_variants = [
        {"KALSHI-ACCESS-KEY": API_KEY_ID},
        {"kalshi-access-key": API_KEY_ID},
        {"Kalshi-Access-Key": API_KEY_ID},
        {"X-KALSHI-ACCESS-KEY": API_KEY_ID},
    ]
    
    timestamp = str(int(datetime.datetime.now(datetime.timezone.utc).timestamp() * 1000))
    message = f"{timestamp}{method}/trade-api/v2{endpoint}"
    signature = private_key.sign(message.encode(), padding.PKCS1v15(), hashes.SHA256())
    sig_b64 = base64.b64encode(signature).decode()
    
    for hv in header_variants:
        key_header = list(hv.keys())[0]
        sig_header = key_header.replace("KEY", "SIGNATURE").replace("key", "signature").replace("Key", "Signature")
        ts_header = key_header.replace("KEY", "TIMESTAMP").replace("key", "timestamp").replace("Key", "Timestamp")
        
        headers = {
            key_header: API_KEY_ID,
            sig_header: sig_b64,
            ts_header: timestamp,
        }
        
        try:
            resp = requests.get(f"{API_BASE}{endpoint}", headers=headers, timeout=10)
            print(f"   {key_header}: {resp.status_code}")
        except Exception as e:
            print(f"   {key_header}: Error")
    
    return False


def check_docs_endpoint():
    """Check if there's a docs or info endpoint"""
    print("\n" + "=" * 60)
    print("CHECKING INFO ENDPOINTS")
    print("=" * 60)
    
    info_endpoints = [
        "/exchange/status",
        "/",
        "/info",
        "/docs",
        "/api-info",
        "/health",
    ]
    
    for ep in info_endpoints:
        try:
            url = f"{API_BASE}{ep}" if ep != "/" else API_BASE
            resp = requests.get(url, timeout=10)
            print(f"   {ep}: {resp.status_code}")
            if resp.status_code == 200 and resp.text:
                try:
                    data = resp.json()
                    print(f"      {json.dumps(data)[:100]}")
                except:
                    print(f"      {resp.text[:100]}")
        except Exception as e:
            print(f"   {ep}: Error - {e}")


if __name__ == "__main__":
    check_docs_endpoint()
    test_signature_variants()
